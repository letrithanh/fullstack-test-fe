...
export default function EventFormWidget() {
    ...
    function descriptionValidation(): InputValidation {
        ...
        if ((descriptionAction.isValueChanged() || isSubmit) && isDescriptionBlank) {
            return {
                isError: true,
    ...
    function maxAttendeeValidation(): InputValidation {
        ...
        if ((maxAttendeeAction.isValueChanged() || isSubmit) && isMaxAttendeeBlank) {
            return {
                isError: true,
        ...
        const inRange = parseInt(assignedMaxAttendee.value) > 0 && parseInt(assignedMaxAttendee.value) <= 100
        if (maxAttendeeAction.isValueChanged() && !inRange) {
            return {
                isError: true,
                errorMessage: <>Max attendee must greater than 0, and less than or equals to 100</>
            }
        }
    ...
    function onSave(event: React.FormEvent<HTMLButtonElement>) {
        ...
        const isValidData = titleAction.isValueChanged() && !titleValidation().isError 
                            && descriptionAction.isValueChanged() && !descriptionValidation().isError
                            && isDateValid(date)
                            && locationAction.isValueChanged() && !locationValidation().isError
                            && maxAttendeeAction.isValueChanged() && !maxAttendeeValidation().isError;
        
        if (isValidData) {
            const eventsClientService = new EventsClientService();
            eventsClientService.createEvent({
